...
object GameMap {
  ...
  def simple(): GameMap = {
    ...
    map.grid.hexes = (for {
      ...
      if (i == 3 && j == 9 || i == 4 && j == 9)
        h.addContent(new SpruceTree1(i, j))
  ...
  def load(dataStream: InputStream): GameMap = {
    val levelString =
      new String(dataStream.readAllBytes(), StandardCharsets.UTF_8)
    ...
    val symbols: Map[(Int, Int), String] = (
      for {
        ...
        l <- lines._1.chars
          .mapToObj(i =>
            new String(List(i.toByte).toArray[Byte], StandardCharsets.UTF_8)
          )
          .toList
          .asScala
          .zipWithIndex
